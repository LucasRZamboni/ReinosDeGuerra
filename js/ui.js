(function () {
  ...
  function reports() {
    ...
    const items=reports.map((r,i)=>r.attacker?`<div class="report-row ${r.read?"":"report-unread"}"><input class="form-check-input report-check" type="checkbox" value="${r.id}"><button class="btn btn-sm report-favorite" data-report-id="${r.id}" title="Favorito">${r.favorite?"★":"☆"}</button><button class="log-entry report-summary ${r.type}" data-report-index="${all.indexOf(r)}"><span class="result-badge">${r.victory?"Vitória":"Derrota"}</span><span><strong>${r.attacker.name}</strong> (${r.attacker.x}|${r.attacker.y}) → <strong>${r.defender.name}</strong> (${r.defender.x}|${r.defender.y})</span><small>${new Date(r.time).toLocaleString("pt-BR")}</small></button><button class="btn btn-sm btn-outline-danger report-delete" data-report-id="${r.id}">Excluir</button></div>`:`<div class="report-row"><div class="log-entry"><strong>${r.title||"Informação"}</strong><small>${r.text||""}</small></div></div>`).join("");
  ...
  function bindDynamic() {
    ...
    if (view === "settings") {
      ...
      const saveSettingsTab = (tab) => {
        ...
        if (tab === "speed") {
          ...
          try { const pointRewards=JSON.parse(String(f.get("pointMilestones")||"[]")), progressTroopRewards=JSON.parse(String(f.get("troopMilestones")||"[]")); G.updateSettings({minimumAttackPopulation:Math.max(1,Number(f.get("minimumAttackPopulation"))||10),freeVillageRules:{barbariansBuild:f.get("barbarianBuild")==="on",barbariansRecruit:f.get("barbarianRecruit")==="on",bonusBuild:f.get("bonusBuild")==="on",bonusRecruit:f.get("bonusRecruit")==="on"},villageMilestones:{enabled:f.get("milestonesEnabled")==="on",pointRewards,progressTroopRewards}}); } catch(err){ notifyUI("JSON inválido nos marcos de evolução.","danger"); return; }
        }
    ...
    if (view === "admin") {
      ...
      $("#adminWorldForm").onsubmit = (e) => {
        ...
        G.updateSettings({
          ...
          newPlayerProtectionHours: Math.max(0,Number(f.get("newPlayerProtectionHours"))||0),
    ...
    if(view==="reports"){ $("#reportTypeFilter")?.addEventListener("change",e=>{localStorage.setItem("rdg_report_filter",e.target.value);render(true)}); $("#reportStateFilter")?.addEventListener("change",e=>{localStorage.setItem("rdg_report_state",e.target.value);render(true)}); document.querySelectorAll(".report-favorite").forEach(b=>b.onclick=()=>G.toggleReportFavorite(b.dataset.reportId)); $("#reportDeleteSelected")?.addEventListener("click",()=>G.deleteReports([...document.querySelectorAll(".report-check:checked")].map(x=>x.value))); }
    if(view==="commands") document.querySelectorAll(".support-withdraw").forEach(b=>b.onclick=()=>G.withdrawSupport(b.dataset.support));
    if (view === "map") bindMap();
  }
  const renderers = { overview, village, army, villages, commands, ranking, map, reports, settings, admin };
    ...
    if (target) {
      ...
      if (!enough)
        ...
        for (let i = 0; i < count; i++)
          f.get("commandType")==="support" ? G.sendSupport(target.id,units) : G.sendAttack(target.id, units, f.get("catapultTarget"));
    } else
      window.dispatchEvent(
        new CustomEvent("game-notify", {
          detail: {
