(function () {
  ...
  function reports() {
    ...
    const items=reports.map((r,i)=>r.attacker?`<div class="report-row ${r.read?"":"report-unread"}"><input class="form-check-input report-check" type="checkbox" value="${r.id}"><button class="btn btn-sm report-favorite" data-report-id="${r.id}" title="Favorito">${r.favorite?"★":"☆"}</button><button class="log-entry report-summary ${r.type}" data-report-id="${r.id}"><span class="result-badge">${r.victory?"Vitória":"Derrota"}</span><span><strong>${r.attacker.name}</strong> (${r.attacker.x}|${r.attacker.y}) → <strong>${r.defender.name}</strong> (${r.defender.x}|${r.defender.y})</span><small>${new Date(r.time).toLocaleString("pt-BR")}</small></button><button class="btn btn-sm btn-outline-danger report-delete" data-report-id="${r.id}">Excluir</button></div>`:`<div class="report-row"><div class="log-entry"><strong>${r.title||"Informação"}</strong><small>${r.text||""}</small></div></div>`).join("");
  ...
  window.addEventListener("game-update", () => {
    ...
    if (["village","army","commands","reports","overview"].includes(view) && !document.activeElement?.matches?.("input,select,textarea")) { render(true); }
  });
