(function () {
  ...
  function knownReportsForVillage(id){return visibleReports().filter(r=>r.targetId===id&&(r.intel?.units||r.defending?.survivors)).sort((a,b)=>b.time-a.time)}
  ...
  document.addEventListener("click", (e) => {
    const sim=e.target.closest?.(".report-sim-spy,.report-sim-survivors");if(sim){const r=visibleReports().find(x=>String(x.id)===String(sim.dataset.reportId));if(r){bootstrap.Modal.getOrCreateInstance($("#reportModal")).hide();view="simulator";const data=sim.classList.contains("report-sim-spy")?r.intel?.units:r.defending?.survivors;setTimeout(()=>{simulator({units:data||{},wall:r.intel?.buildings?.wall||0,label:`Dados do relatório de ${new Date(r.time).toLocaleString("pt-BR")} — podem estar desatualizados`});bindDynamic()},100);}return;}
