(function () {
  const C = GAME_CONFIG,
    ...
    fmtCompact = (n) => new Intl.NumberFormat("pt-BR",{notation:"compact",maximumFractionDigits:1}).format(Math.floor(Number(n)||0));
  ...
  function header() {
    ...
    renderIncomingAttackAlert();
    const mineIdsForNav=new Set((G.owned?.()||[]).map(v=>v.id)), unreadForNav=(visibleReports?.()||[]).filter(r=>!r.read).length, incomingForNav=(G.state.movements||[]).filter(m=>m.outbound&&m.kind!=="support"&&mineIdsForNav.has(m.targetId)).length; const rc=$("#navReportsCount"),cc=$("#navCommandsCount"); if(rc){rc.textContent=unreadForNav;rc.classList.toggle("d-none",!unreadForNav)} if(cc){cc.textContent=incomingForNav;cc.classList.toggle("d-none",!incomingForNav)};
      return;
    }
    updateLive();
  ...
  function renderIncomingAttackAlert() {
    const box=$("#attackAlertPersistent");
    const p=G.playerPreferences?.();
    if(!p?.attackAlert){ box?.remove(); return; }
    const mine=new Set(G.owned().map(v=>v.id));
    const incoming=(G.state.movements||[]).filter(m=>m.outbound&&m.kind!=="support"&&mine.has(m.targetId)&&!G.isMine(G.state.villages[m.fromId])).sort((a,b)=>a.end-b.end);
    if(!incoming.length){ box?.remove(); return; }
    const m=incoming[0],from=G.state.villages[m.fromId],to=G.state.villages[m.targetId];
    const pop=Object.entries(m.units||{}).reduce((n,[k,q])=>n+(C.units[k]?.population||0)*(Number(q)||0),0);
    const label=pop<100?"Muito fraco":pop<500?"Fraco":pop<1500?"Médio":pop<4000?"Forte":"Muito forte";
    const html=`<div id="attackAlertPersistent" class="alert alert-danger persistent-attack-alert"><strong>⚠ Ataque a caminho</strong><div>${from?.name||"Origem desconhecida"} (${from?.x??"?"}|${from?.y??"?"}) → ${to?.name||"Sua aldeia"} (${to?.x??"?"}|${to?.y??"?"})</div><small>Chegada: ${new Date(m.end).toLocaleString("pt-BR")}${p.attackAlertStrength!==false?` · Força estimada: <strong>${label}</strong>`:""}</small>${incoming.length>1?`<div class="small mt-1">+ ${incoming.length-1} outro(s) ataque(s) chegando</div>`:""}</div>`;
    if(box) box.outerHTML=html; else $("#alerts")?.insertAdjacentHTML("afterbegin",html);
  }
  function updateLive() {
    const v = G.active();
    if (!v) return;
    const cap = G.cap(v);
    const mobileNumbers=window.matchMedia?.("(max-width: 575px)")?.matches, rf=n=>mobileNumbers?fmtCompact(n):fmt(n);
    $("#resourceBar").innerHTML =
      `<button class="resource-pill resource-info" data-resource="wood" title="Madeira ${fmt(v.resources.wood)} / ${fmt(cap)}">🪵 <strong>${rf(v.resources.wood)}/${rf(cap)}</strong></button><button class="resource-pill resource-info" data-resource="clay" title="Argila ${fmt(v.resources.clay)} / ${fmt(cap)}">🧱 <strong>${rf(v.resources.clay)}/${rf(cap)}</strong></button><button class="resource-pill resource-info" data-resource="iron" title="Ferro ${fmt(v.resources.iron)} / ${fmt(cap)}">⛓ <strong>${rf(v.resources.iron)}/${rf(cap)}</strong></button><span class="resource-pill" title="População ${fmt(G.population(v))} / ${fmt(G.popCap(v))}">👥 <strong>${rf(G.population(v))}/${rf(G.popCap(v))}</strong></span><span class="resource-pill" title="${fmt(G.points(v))} pontos">★ <strong>${rf(G.points(v))}</strong></span>`;
  ...
  function reportModal(r) {
    if(!r) return;
    // Relatórios informativos/legados não possuem estrutura de batalha.
    if(!r.attacker || !r.defender || !r.attacking || !r.defending){
      $("#reportModalTitle").textContent = r.title || "Relatório";
      $("#reportModalBody").innerHTML = `<section class="report-section"><h3 class="h5">${r.title||"Informação"}</h3><p>${r.text||"Este relatório não possui detalhes adicionais."}</p><small class="text-secondary">${r.time?new Date(r.time).toLocaleString("pt-BR"):""}</small></section>`;
  ...
  function reports() {
    const p=G.playerPreferences?.()||{}, allowed=p.reportTypes||{attack:true,defense:true,support:true,spy:true,conquest:true,info:true};
    const allRaw=visibleReports(), classify=r=>r.conquered?"conquest":r.intel?"spy":r.type==="support"?"support":r.attacker?(r.defenderPlayerId===G.currentPlayerId()?"defense":"attack"):"info";
    const all=allRaw.filter(r=>allowed[classify(r)]!==false), filter=localStorage.getItem("rdg_report_filter")||"all", stateFilter=localStorage.getItem("rdg_report_state")||"all";
    ...
    const items=reports.map(r=>r.attacker?`<div class="report-row ${r.read?"":"report-unread"}"><input class="form-check-input report-check" type="checkbox" value="${r.id}"><button class="btn btn-sm report-favorite" data-report-id="${r.id}" title="Favorito">${r.favorite?"★":"☆"}</button><button class="log-entry report-summary ${r.type||""}" data-report-id="${r.id}"><span class="result-badge">${r.victory?"Vitória":"Derrota"}</span><span><strong>${r.attacker.name}</strong> (${r.attacker.x}|${r.attacker.y}) → <strong>${r.defender.name}</strong> (${r.defender.x}|${r.defender.y})</span><small>${new Date(r.time).toLocaleString("pt-BR")}</small></button><button class="btn btn-sm btn-outline-danger report-delete" data-report-id="${r.id}">Excluir</button></div>`:`<div class="report-row ${r.read?"":"report-unread"}"><input class="form-check-input report-check" type="checkbox" value="${r.id}"><button class="log-entry report-summary" data-report-id="${r.id}"><strong>${r.title||"Informação"}</strong><small>${r.text||""}</small></button><button class="btn btn-sm btn-outline-danger report-delete" data-report-id="${r.id}">Excluir</button></div>`).join("");
    $("#view-reports").innerHTML=`<div class="section-title"><div><div class="eyebrow">Crônicas militares</div><h2>Relatórios</h2></div><span class="metric-chip">${allRaw.filter(r=>!r.read).length} não lidos</span></div><section class="panel p-2 mb-3 d-flex flex-wrap gap-2"><select id="reportTypeFilter" class="form-select form-select-sm" style="width:auto"><option value="all">Todos</option><option value="attack">Ataques</option><option value="defense">Defesas</option><option value="support">Apoios</option><option value="spy">Espionagem</option><option value="conquest">Conquistas</option><option value="info">Outros</option></select><select id="reportStateFilter" class="form-select form-select-sm" style="width:auto"><option value="all">Todos</option><option value="unread">Não lidos</option><option value="read">Lidos</option><option value="favorite">Favoritos</option></select><button id="reportDeleteSelected" class="btn btn-sm btn-outline-danger">Excluir selecionados</button>${allRaw.length?'<button id="deleteAllReports" class="btn btn-outline-danger btn-sm">Excluir todos</button>':""}<button id="reportPrefsToggle" class="btn btn-sm btn-outline-secondary">Preferências</button></section><section id="reportPrefs" class="panel p-3 mb-3 d-none"><strong>Tipos exibidos</strong><div class="d-flex gap-3 flex-wrap mt-2">${["attack","defense","support","spy","conquest","info"].map(k=>`<label class="form-check"><input class="form-check-input report-pref" type="checkbox" value="${k}" ${allowed[k]!==false?"checked":""}> ${k==="attack"?"Ataques":k==="defense"?"Defesas":k==="support"?"Apoios":k==="spy"?"Espionagem":k==="conquest"?"Conquistas":"Outros"}</label>`).join("")}</div><small class="text-secondary">Ocultar um tipo não apaga os relatórios.</small></section>${items||'<p class="text-secondary">Nenhum relatório neste filtro.</p>'}`;
  ...
  function hub(){
    const p=G.playerPreferences?.()||{}, selected=new Set(p.quickMenu||[]);
    const items=[
      ["overview","◈","Visão Geral","Resumo do reino, produção e movimentos."],
      ["reports","☷","Relatórios","Batalhas, espionagem, conquistas e avisos."],
      ["villages","♜","Aldeias","Gestão e filtros das suas aldeias."],
      ["army","⚔","Exército","Tropas da aldeia ativa e treinamento."],
      ["market","⚖","Mercado","Trocas de recursos."],
      ["ranking","🏆","Ranking","Classificações do mundo."],
      ["achievements","✦","Conquistas","Metas e recompensas."],
      ["simulator","⚔","Simulador","Teste o resultado de combates."],
      ["settings","⚙","Personalização","Alertas e preferências da interface."],
    ];
    $("#view-hub").innerHTML=`<div class="section-title"><div><div class="eyebrow">Acesso rápido</div><h2>Central do Jogo</h2><p class="view-subtitle">Os recursos essenciais continuam no menu lateral. Use esta área para as funções menos frequentes e escolha seus próprios atalhos.</p></div><button id="hubBackBtn" class="btn btn-outline-light">← Voltar</button></div>
    <section class="panel p-3 mb-3"><div class="d-flex justify-content-between gap-2 flex-wrap align-items-center"><div><strong>Atalhos do menu lateral</strong><div class="small text-secondary">Aldeia, Mapa e Comandos são fixos. Marque abaixo o que também deve aparecer na tela principal.</div></div><button id="hubPauseBtn" class="btn btn-sm ${G.state.paused?"btn-success":"btn-outline-warning"}">${G.state.paused?"▶ Continuar mundo":"⏸ Pausar mundo"}</button></div><div class="hub-shortcut-options mt-3">${items.filter(x=>x[0]!=="settings").map(([key,icon,name])=>`<label class="form-check hub-shortcut-check"><input class="form-check-input hub-quick-toggle" type="checkbox" value="${key}" ${selected.has(key)?"checked":""}> ${icon} ${name}</label>`).join("")}</div><button id="saveHubShortcuts" class="btn btn-warning btn-sm mt-3">Salvar atalhos</button></section>
    <div class="hub-menu-grid">${items.map(([key,icon,name,desc])=>`<button class="panel hub-menu-card" data-hub-view="${key}"><span>${icon}</span><strong>${name}</strong><small>${desc}</small></button>`).join("")}${isAdmin()?`<button class="panel hub-menu-card admin-hub-card" data-hub-view="admin"><span>★</span><strong>Administração</strong><small>Configuração e ferramentas do mundo.</small></button>`:""}</div>`;
  }

  function playerSettings() {
    const p=G.playerPreferences();
    $("#view-settings").innerHTML=`<div class="section-title"><div><div class="eyebrow">Preferências pessoais</div><h2>Personalização</h2><p class="view-subtitle">Estas opções afetam apenas a sua interface.</p></div></div><section class="panel p-4"><form id="personalizationForm"><div class="row g-3"><div class="col-md-6"><div class="rule-card"><strong>Alertas de ataque</strong><label class="form-check form-switch mt-2"><input class="form-check-input" name="attackAlert" type="checkbox" ${p.attackAlert?"checked":""}> Mostrar alerta em tela</label><div class="setting-help">Desativado por padrão. Exibe origem, destino, chegada e uma estimativa de força.</div><label class="form-check mt-2"><input class="form-check-input" name="attackAlertStrength" type="checkbox" ${p.attackAlertStrength!==false?"checked":""}> Mostrar estimativa de força</label></div></div><div class="col-md-6"><div class="rule-card"><strong>Mapa e notificações</strong><label class="form-check form-switch mt-2"><input class="form-check-input" name="mapCommandIndicators" type="checkbox" ${p.mapCommandIndicators!==false?"checked":""}> Mostrar indicadores de comandos no mapa</label><label class="form-check form-switch mt-2"><input class="form-check-input" name="compactNotifications" type="checkbox" ${p.compactNotifications?"checked":""}> Notificações compactas</label></div></div></div><button class="btn btn-warning mt-3">Salvar personalização</button></form></section>`;
  }
  function settings() {
    if(!isAdmin()) return playerSettings();
        ...
        .map((cell) => {
          ...
          if (v) {
            ...
            const incoming = G.state.movements.some(
                (m) => m.outbound && m.targetId === v.id && m.kind !== "support" && m.kind !== "supportReturn",
              ),
            ...
            const showMovementMarks = G.playerPreferences?.().mapCommandIndicators!==false;
            const movementMarks = protectionMark + (showMovementMarks ? `${incoming ? '<i class="map-movement attack-mark" title="Ataque a caminho">⚔</i>' : ""}${returning ? '<i class="map-movement return-mark" title="Tropas retornando">↩</i>' : ""}` : "");
  ...
  function villages() {
    const original=G.owned(), groups=G.villageGroups?.()||{}, active=G.active(), first=G.state.players?.[G.currentPlayerId()]?.firstVillageId;
    const prefs={search:localStorage.getItem("rdg_v_search")||"",min:Number(localStorage.getItem("rdg_v_min")||0),max:Number(localStorage.getItem("rdg_v_max")||0),role:localStorage.getItem("rdg_v_role")||"all",noble:localStorage.getItem("rdg_v_noble")||"all",group:localStorage.getItem("rdg_v_group")||"all",order:localStorage.getItem("rdg_v_order")||"conquest"};
    const attackKeys=new Set(["axe","light","ram","catapult","noble"]), defenseKeys=new Set(["spear","sword","archer","heavy"]);
    const roleScore=(v,set)=>Object.entries(v.units||{}).reduce((n,[k,q])=>n+(set.has(k)?(C.units[k]?.population||1)*(Number(q)||0):0),0);
    let mine=original.filter(v=>{const pts=G.points(v),q=prefs.search.toLowerCase();if(q&&!(`${v.name} ${v.x}|${v.y}`.toLowerCase().includes(q)))return false;if(prefs.min&&pts<prefs.min)return false;if(prefs.max&&pts>prefs.max)return false;if(prefs.noble==="ready"&&!(v.units?.noble>0))return false;if(prefs.noble==="academy"&&!(v.buildings?.academy>0))return false;if(prefs.noble==="none"&&(v.units?.noble>0||v.buildings?.academy>0))return false;const a=roleScore(v,attackKeys),d=roleScore(v,defenseKeys);if(prefs.role==="attack"&&a<=d)return false;if(prefs.role==="defense"&&d<a)return false;if(prefs.group!=="all"&&(groups[v.id]||"")!==prefs.group)return false;return true;});
    const acquisition=v=>v.id===first?0:Number(v.conqueredAt||v.createdAt||0);
    if(prefs.order==="points-desc")mine.sort((a,b)=>G.points(b)-G.points(a));else if(prefs.order==="points-asc")mine.sort((a,b)=>G.points(a)-G.points(b));else if(prefs.order==="name")mine.sort((a,b)=>a.name.localeCompare(b.name,"pt-BR"));else if(prefs.order==="distance")mine.sort((a,b)=>Math.hypot(a.x-(active?.x||0),a.y-(active?.y||0))-Math.hypot(b.x-(active?.x||0),b.y-(active?.y||0)));else mine.sort((a,b)=>acquisition(a)-acquisition(b));
    const totalPoints=original.reduce((n,v)=>n+G.points(v),0), groupNames=[...new Set(Object.values(groups).filter(Boolean))].sort();
    const cards=mine.map(v=>{const cap=G.cap(v),bq=v.buildQueue?.[0],tq=v.trainQueue?.[0],troops=Object.entries(v.units||{}).filter(([,q])=>q>0).slice(0,6).map(([k,q])=>`${C.units[k]?.icon||""}${fmt(q)}`).join(" ")||"—",a=roleScore(v,attackKeys),d=roleScore(v,defenseKeys),role=a>d?"Ataque":"Defesa",group=groups[v.id]||"Sem grupo";return `<article class="panel p-3 village-manage-card"><div class="d-flex gap-2 align-items-start"><input class="form-check-input village-bulk-check mt-1" type="checkbox" value="${v.id}"><div class="flex-grow-1"><div class="d-flex justify-content-between gap-2 flex-wrap"><strong>${v.name}</strong><span>${G.points(v)} pts · ${v.x}|${v.y}</span></div><div class="small text-secondary">${role} · ${group}${v.units?.noble?` · 👑 ${v.units.noble} nobre(s)`:v.buildings?.academy?" · Academia pronta":""}</div><div class="small mt-2">🪵${fmt(v.resources.wood)}/${fmt(cap)} · 🧱${fmt(v.resources.clay)}/${fmt(cap)} · ⛓${fmt(v.resources.iron)}/${fmt(cap)} · 👥${G.population(v)}/${G.popCap(v)}</div><div class="small text-secondary mt-1">${troops}</div><div class="small mt-2">🏗 ${bq?`${C.buildings[bq.building]?.name} (${remaining(bq.end)})`:"Fila livre"} · ⚔ ${tq?`${C.units[tq.unit]?.name} ${tq.trained||0}/${tq.amount}`:"Fila livre"}</div><div class="d-flex gap-1 flex-wrap mt-2"><button class="btn btn-sm btn-warning village-open" data-id="${v.id}">Abrir</button><button class="btn btn-sm btn-outline-light village-map" data-id="${v.id}">Mapa</button></div></div></div></article>`}).join("");
    const buildOpts=Object.entries(C.buildings).map(([k,b])=>`<option value="${k}">${b.name}</option>`).join("");
    $("#view-villages").innerHTML=`<div class="section-title"><div><div class="eyebrow">Gestão do reino</div><h2>Minhas aldeias</h2></div><div class="d-flex gap-2 flex-wrap"><span class="badge text-bg-warning">★ ${fmt(totalPoints)} pontos</span><span class="badge text-bg-secondary">♜ ${original.length} aldeias</span><span class="badge text-bg-info">${mine.length} visíveis</span></div></div><section class="panel p-3 mb-3"><div class="row g-2"><div class="col-md-3"><label>Buscar</label><input id="villageFilterSearch" class="form-control form-control-sm" value="${prefs.search}" placeholder="Nome ou X|Y"></div><div class="col-md-2"><label>Pontos mín.</label><input id="villageFilterMin" type="number" class="form-control form-control-sm" value="${prefs.min||""}"></div><div class="col-md-2"><label>Pontos máx.</label><input id="villageFilterMax" type="number" class="form-control form-control-sm" value="${prefs.max||""}"></div><div class="col-md-2"><label>Perfil</label><select id="villageFilterRole" class="form-select form-select-sm"><option value="all">Todos</option><option value="attack">Ataque</option><option value="defense">Defesa</option></select></div><div class="col-md-3"><label>Nobres</label><select id="villageFilterNoble" class="form-select form-select-sm"><option value="all">Todos</option><option value="ready">Nobre disponível</option><option value="academy">Com Academia</option><option value="none">Sem nobre/Academia</option></select></div><div class="col-md-3"><label>Grupo</label><select id="villageFilterGroup" class="form-select form-select-sm"><option value="all">Todos os grupos</option>${groupNames.map(x=>`<option value="${x}">${x}</option>`).join("")}</select></div><div class="col-md-3"><label>Ordenar</label><select id="villageFilterOrder" class="form-select form-select-sm"><option value="conquest">1ª → última conquistada</option><option value="points-desc">Pontos: maior → menor</option><option value="points-asc">Pontos: menor → maior</option><option value="name">Nome</option><option value="distance">Distância da aldeia ativa</option></select></div><div class="col-md-3 d-flex align-items-end"><button id="villageClearFilters" class="btn btn-sm btn-outline-secondary w-100">Limpar filtros</button></div></div></section><section class="panel p-3 mb-3 village-bulk-toolbar"><div class="d-flex flex-wrap gap-2 align-items-end"><button id="villageSelectAll" class="btn btn-sm btn-outline-light">Selecionar visíveis</button><button id="villageClearAll" class="btn btn-sm btn-outline-secondary">Limpar seleção</button><label class="flex-grow-1">Construir<select id="villageBulkBuilding" class="form-select form-select-sm"><option value="">Escolha…</option>${buildOpts}</select></label><button id="villageBulkBuild" class="btn btn-sm btn-warning">Construir selecionadas</button><button class="btn btn-sm btn-outline-warning village-bulk-preset" data-preset="defense">Treinar Defesa</button><button class="btn btn-sm btn-outline-danger village-bulk-preset" data-preset="attack">Treinar Ataque</button><button class="btn btn-sm btn-outline-info village-bulk-preset" data-preset="custom">Usar personalizado</button><button id="editVillageCustom" class="btn btn-sm btn-outline-light">Editar personalizado</button></div><div class="d-flex gap-2 flex-wrap mt-2"><input id="villageGroupName" class="form-control form-control-sm" style="max-width:220px" placeholder="Grupo: Ataque, Defesa…"><button id="villageSetGroup" class="btn btn-sm btn-outline-info">Aplicar grupo</button><button id="villageClearGroup" class="btn btn-sm btn-outline-secondary">Remover grupo</button></div></section><div class="village-management-grid">${cards||'<p class="text-secondary">Nenhuma aldeia corresponde aos filtros.</p>'}</div>`;
    $("#villageFilterRole").value=prefs.role;$("#villageFilterNoble").value=prefs.noble;$("#villageFilterGroup").value=prefs.group;$("#villageFilterOrder").value=prefs.order;
  }
  function overview(){
    const mine=G.owned()||[],pts=mine.reduce((n,v)=>n+G.points(v),0),prod={wood:0,clay:0,iron:0},troops={};let pop=0,cap=0,builds=0,trains=0;
    mine.forEach(v=>{["wood","clay","iron"].forEach(r=>prod[r]+=(Number(G.prod(v,r))||0)*60);Object.entries(v.units||{}).forEach(([k,q])=>troops[k]=(troops[k]||0)+(Number(q)||0));pop+=G.population(v);cap+=G.popCap(v);builds+=(v.buildQueue||[]).length;trains+=(v.trainQueue||[]).length;});
    const ids=new Set(mine.map(v=>v.id)),moves=G.state.movements||[],incoming=moves.filter(m=>m.outbound&&m.kind!=="support"&&ids.has(m.targetId)),outgoing=moves.filter(m=>m.outbound&&ids.has(m.fromId)&&m.kind!=="support"),supports=moves.filter(m=>m.kind==="support"&&(ids.has(m.fromId)||ids.has(m.targetId))),rec=(G.state.recurringAttacks||[]).filter(x=>x.ownerId===G.currentPlayerId()&&x.status==="active").length,troopTotal=Object.values(troops).reduce((a,b)=>a+b,0);
    const nextIncoming=[...incoming].sort((a,b)=>a.end-b.end).slice(0,5).map(m=>{const f=G.state.villages[m.fromId],t=G.state.villages[m.targetId];return `<tr><td>${f?.name||"?"}</td><td>${t?.name||"?"}</td><td>${remaining(m.end)}</td></tr>`}).join("");
    $("#view-overview").innerHTML=`<div class="section-title"><div><div class="eyebrow">Painel do reino</div><h2>Visão Geral</h2><p class="view-subtitle">Resumo operacional do seu domínio.</p></div></div><div class="overview-metrics"><div class="panel p-3"><small>Pontos</small><strong>★ ${fmt(pts)}</strong></div><div class="panel p-3"><small>Aldeias</small><strong>♜ ${mine.length}</strong></div><div class="panel p-3"><small>Tropas</small><strong>⚔ ${fmt(troopTotal)}</strong></div><div class="panel p-3"><small>População</small><strong>♟ ${fmt(pop)}/${fmt(cap)}</strong></div><div class="panel p-3"><small>Ataques chegando</small><strong>⚠ ${incoming.length}</strong></div><div class="panel p-3"><small>Ataques enviados</small><strong>➤ ${outgoing.length}</strong></div><div class="panel p-3"><small>Apoios</small><strong>🛡 ${supports.length}</strong></div><div class="panel p-3"><small>Programações</small><strong>⏱ ${rec}</strong></div></div><div class="row g-3 mt-1"><div class="col-lg-6"><section class="panel p-3 h-100"><h3 class="h5">Produção total por hora</h3><div class="d-flex gap-4 flex-wrap"><span>🪵 ${fmt(prod.wood)}/h</span><span>🧱 ${fmt(prod.clay)}/h</span><span>⛏ ${fmt(prod.iron)}/h</span></div><div class="small text-secondary mt-3">Filas: ${builds} construção(ões) · ${trains} treinamento(s)</div></section></div><div class="col-lg-6"><section class="panel p-3 h-100"><h3 class="h5">Próximos ataques recebidos</h3><div class="table-responsive"><table class="table table-dark table-sm mb-0"><thead><tr><th>Origem</th><th>Destino</th><th>Chegada</th></tr></thead><tbody>${nextIncoming||'<tr><td colspan=3 class="text-secondary">Nenhum ataque chegando.</td></tr>'}</tbody></table></div></section></div></div>`;
  }
  function commands(){
    const mine=G.owned(),mineIds=new Set(mine.map(v=>v.id)),admin=isAdmin(),allMoves=admin?G.state.movements:(G.state.movements||[]).filter(m=>mineIds.has(m.fromId)||mineIds.has(m.targetId)),classify=m=>m.kind==="support"||m.kind==="supportReturn"?"support":m.scheduled?"scheduled":m.outbound?(mineIds.has(m.fromId)?"outgoing":mineIds.has(m.targetId)?"incoming":"other"):"return",cfilter=localStorage.getItem("rdg_command_filter")||"all",visible=[...allMoves].filter(m=>cfilter==="all"||classify(m)===cfilter).sort((a,b)=>a.end-b.end);
    const rows=visible.map(m=>{const from=G.state.villages[m.fromId],to=G.state.villages[m.targetId],mineFrom=mineIds.has(m.fromId),mineTo=mineIds.has(m.targetId),kind=m.scheduled?"Agendado":m.kind==="support"?"Apoio":m.kind==="supportReturn"?"Retorno de apoio":m.outbound?(mineFrom?"Ataque enviado":mineTo?"Ataque recebido":"Ataque"):"Retorno",reveal=admin||mineFrom||!m.outbound,units=reveal?Object.entries(m.units||{}).filter(([,q])=>q).map(([k,q])=>`${C.units[k]?.icon||""}${q}`).join(" "):"Tropas ocultas";return `<tr><td><strong>${kind}</strong></td><td><button class="command-village-link" data-village-id="${from?.id||""}">${from?.name||"?"} (${from?.x}|${from?.y})</button></td><td><button class="command-village-link" data-village-id="${to?.id||""}">${to?.name||"?"} (${to?.x}|${to?.y})</button></td><td>${units}</td><td>${m.scheduled?`<small>Saída: ${new Date(m.departAt).toLocaleString("pt-BR")}</small><br>`:""}<span class="countdown" data-countdown="${m.end}">${remaining(m.end)}</span>${m.scheduled&&mineFrom?`<br><button class="btn btn-sm btn-outline-danger cancel-scheduled mt-1" data-movement="${m.id}">Cancelar</button>`:""}</td></tr>`}).join("");
    ...
    const recs=(G.state.recurringAttacks||[]).filter(x=>admin||x.ownerId===G.currentPlayerId()),finished=recs.filter(x=>["finished","cancelled"].includes(x.status)).length,targets=Object.values(G.state.villages).filter(v=>!G.isMine(v));
    const targetRows=targets.map(v=>{const type=(v.bonusType&&v.bonusType!=="none")?"bonus":(v.owner?"enemy":"barbarian"),q=v.x<0?(v.y<0?"NW":"SW"):(v.y<0?"NE":"SE"),d=mine.length?Math.min(...mine.map(o=>Math.hypot(v.x-o.x,v.y-o.y))):0;return `<label class="form-check recurring-target-row" data-type="${type}" data-q="${q}" data-distance="${d}" data-search="${(v.name+" "+v.x+"|"+v.y).toLowerCase()}"><input class="form-check-input recurring-target" type="checkbox" value="${v.id}"> ${v.name} (${v.x}|${v.y}) · ${type==="bonus"?"Bônus":type==="barbarian"?"Bárbara":"Inimigo"} · ${d.toFixed(1)} campos</label>`}).join("");
    $("#view-commands").innerHTML=`<div class="section-title"><div><div class="eyebrow">Movimentações militares</div><h2>${admin?"Central global de comandos":"Central de comandos"}</h2><p class="view-subtitle">Ataques, apoios, retornos e programações.</p></div><span class="metric-chip">⚔ ${allMoves.length} ativos</span></div><section class="panel p-2 mb-3"><select id="commandFilter" class="form-select form-select-sm" style="width:auto"><option value="all">Todos</option><option value="incoming">Ataques chegando</option><option value="outgoing">Ataques enviados</option><option value="scheduled">Agendados</option><option value="support">Apoios</option><option value="return">Retornos</option></select></section><section class="panel p-3"><div class="table-responsive commands-scroll"><table class="table table-dark align-middle commands-table"><thead><tr><th>Tipo</th><th>Origem</th><th>Destino</th><th>Tropas</th><th>Chegada</th></tr></thead><tbody>${rows||'<tr><td colspan="5" class="text-secondary">Nenhum comando neste filtro.</td></tr>'}</tbody></table></div></section>${!admin?`<section class="panel p-3 mt-3"><h3 class="h5">Sincronizar ataques</h3><form id="syncAttackForm"><div class="row g-2"><div class="col-md-4"><label>Destino X</label><input name="x" type="number" class="form-control" required></div><div class="col-md-4"><label>Destino Y</label><input name="y" type="number" class="form-control" required></div><div class="col-md-4"><label>Chegada</label><input name="at" type="datetime-local" step="1" class="form-control" required></div><div class="col-md-4"><label>Percentual</label><select name="ratio" class="form-select"><option value=".25">25%</option><option value=".5">50%</option><option value=".75">75%</option><option value="1">100%</option></select></div></div><div class="sync-origin-list mt-3">${mine.map(v=>`<label class="form-check"><input class="form-check-input sync-origin" type="checkbox" value="${v.id}"> ${v.name} (${v.x}|${v.y})</label>`).join("")}</div><button class="btn btn-warning mt-3">Agendar selecionadas</button></form></section>`:""}<section class="panel p-3 mt-3 recurring-attacks-panel"><div class="d-flex justify-content-between gap-2 flex-wrap"><div><div class="eyebrow">Automação militar</div><h3 class="h4">⏱ Ataques recorrentes</h3></div>${finished?`<button id="clearFinishedRecurring" class="btn btn-sm btn-outline-danger align-self-start">Limpar finalizados (${finished})</button>`:""}</div><form id="recurringAttackForm"><div class="mb-2"><label class="form-label">Destinos</label><div class="row g-2 mb-2"><div class="col-md-2"><select id="recurringTargetType" class="form-select"><option value="all">Todos</option><option value="enemy">Inimigos</option><option value="barbarian">Bárbaras</option><option value="bonus">Bônus</option></select></div><div class="col-md-2"><select id="recurringTargetQuadrant" class="form-select"><option value="all">Todos quadrantes</option><option value="NW">NW</option><option value="NE">NE</option><option value="SW">SW</option><option value="SE">SE</option></select></div><div class="col-md-3"><input id="recurringTargetSearch" class="form-control" placeholder="Nome ou X|Y"></div><div class="col-md-2"><input id="recurringTargetLimit" type="number" min="1" class="form-control" placeholder="Qtd. alvos"></div><div class="col-md-3 d-grid"><button type="button" id="recurringSelectVisible" class="btn btn-outline-warning">Selecionar mais próximos</button></div></div><div id="recurringTargetList" class="sync-origin-list recurring-target-list">${targetRows}</div><small class="text-secondary">Ordenação automática do mais próximo para o mais distante. A quantidade limita quantos alvos serão selecionados.</small></div><div class="row g-2"><div class="col-md-3"><label>Destino X (manual)</label><input name="x" type="number" class="form-control"></div><div class="col-md-3"><label>Destino Y (manual)</label><input name="y" type="number" class="form-control"></div><div class="col-md-3"><label>A cada (min)</label><input name="interval" type="number" min="1" value="5" class="form-control"></div><div class="col-md-3"><label>Durante (min)</label><input name="duration" type="number" min="0" value="60" class="form-control"></div></div><div class="mt-2"><strong>Origens</strong>${mine.map(v=>`<label class="form-check"><input class="form-check-input recurring-origin" type="checkbox" value="${v.id}"> ${v.name} (${v.x}|${v.y})</label>`).join("")}</div><div class="sim-grid mt-2">${Object.entries(C.units).map(([k,x])=>`<label class="sim-unit"><span>${x.icon} ${x.name}</span><input name="u-${k}" type="number" min="0" value="0" class="form-control form-control-sm"></label>`).join("")}</div><button class="btn btn-warning mt-2">Criar programação</button></form><div class="mt-3">${recs.map(x=>`<div class="queue-item"><strong>${x.status}</strong> · a cada ${Math.round(x.intervalMs/60000)} min · próximo: ${x.status==="active"?new Date(x.nextAt).toLocaleTimeString("pt-BR"):"—"} · tentativas ${x.attempts} · enviados ${x.sent} · falhas ${x.failed}<div class="mt-1">${!["finished","cancelled"].includes(x.status)?`<button class="btn btn-sm btn-outline-warning recurring-action" data-id="${x.id}" data-action="${x.status==="paused"?"resume":"pause"}">${x.status==="paused"?"Continuar":"Pausar"}</button> <button class="btn btn-sm btn-outline-danger recurring-action" data-id="${x.id}" data-action="cancel">Cancelar</button>`:`<button class="btn btn-sm btn-outline-danger recurring-delete" data-id="${x.id}">Excluir</button>`}</div></div>`).join("")||'<small class="text-secondary">Nenhuma programação.</small>'}</div></section><section class="panel p-3 mt-3"><h3 class="h5">Apoios estacionados</h3><div class="table-responsive supports-scroll"><table class="table table-dark"><thead><tr><th>Origem</th><th>Destino</th><th>Tropas</th><th>Ação</th></tr></thead><tbody>${stationed||"<tr><td colspan=4>Nenhum apoio estacionado.</td></tr>"}</tbody></table></div></section>`;
    $("#commandFilter").value=cfilter;
  }
  function ranking(){
    ...
    Object.entries(G.state.combatStats||{}).forEach(([id,c])=>{const x=stats[id]||(stats[id]={id,points:0,villages:0,type:id.startsWith("ai-")?"enemy":"player",name:names[id]||id});x.attack=Number(c.attackerPoints)||0;x.defense=Number(c.defenderPoints)||0;x.support=Number(c.supporterPoints)||0;x.loot=Number(c.resourcesPlundered)||0;x.conquests=Number(c.conquests)||0;});
    Object.values(stats).forEach(x=>{x.attack||=0;x.defense||=0;x.support||=0;x.loot||=0;x.conquests||=0;});
    const base=Object.values(stats),sorted=k=>[...base].sort((a,b)=>b[k]-a[k]||b.points-a.points),tabs=[["points","Pontos"],["attack","Atacantes"],["defense","Defensores"],["support","Apoiadores"],["loot","Ladrões"],["conquests","Conquistadores"]];
    ...
    $("#view-ranking").innerHTML=`<div class="section-title"><div><div class="eyebrow">Classificação do mundo</div><h2>Rankings</h2><p class="view-subtitle">Os principais rankings competitivos do mundo.</p></div></div><div class="ranking-tabs panel p-2 mb-3 d-flex flex-wrap gap-1">${tabs.map(([k,l],i)=>`<button class="btn btn-sm ranking-tab ${i===0?"active":""}" data-rank="${k}">${l}</button>`).join("")}</div>${tabs.map(([k,l],i)=>`<section class="panel p-3 ranking-pane ${i?"d-none":""}" data-rank-pane="${k}">${table(sorted(k),k,k==="loot"?"Recursos saqueados":k==="conquests"?"Aldeias conquistadas":k==="points"?"Pontos":`Pontos de ${l.toLowerCase()}`)}</section>`).join("")}`;
  }
  function admin() {
  ...
  function bindDynamic() {
    ...
    if (view === "settings") {
      $("#personalizationForm")?.addEventListener("submit",e=>{e.preventDefault();const f=new FormData(e.currentTarget);G.updatePlayerPreferences({attackAlert:f.get("attackAlert")==="on",attackAlertStrength:f.get("attackAlertStrength")==="on",mapCommandIndicators:f.get("mapCommandIndicators")==="on",compactNotifications:f.get("compactNotifications")==="on"});notifyUI("Personalização salva.","success");render(true);});
    ...
    if (view === "admin") {
      ...
      $("#adminWorldForm").onsubmit = (e) => {
        ...
        G.updateSettings({
          ...
          respawnRules:{playerMode:f.get("playerRespawnMode")||"optional",enemyMode:f.get("enemyRespawnMode")||"disabled",enemyDelayMinutes:Math.max(0,Number(f.get("enemyRespawnDelay"))||0)},
      ...
      $("#saveSpawnSettings")?.addEventListener("click",()=>{ const f=new FormData($("#adminWorldForm")); G.updateSettings({barbarianSpawn:{enabled:f.get("spawnEnabled")==="on",intervalMinutes:Math.max(1,Number(f.get("spawnInterval"))||30),maxNewVillages:Math.max(0,Number(f.get("spawnMax"))||0),perCycle:Math.max(1,Number(f.get("spawnPerCycle"))||1),bonusChance:Math.max(0,Math.min(100,Number(f.get("spawnBonus"))||0)),maximized:f.get("spawnMaximized")==="on"},worldGeneration:{distribution:f.get("spawnDistribution")||"centerOut",preserveTerrain:f.get("spawnPreserveTerrain")==="on",densityProfile:f.get("spawnDensityProfile")||"medium",initialRadius:Math.max(3,Number(f.get("spawnInitialRadius"))||22),minVillageDistance:Math.max(0,Number(f.get("spawnMinDistance"))||1.5),expansionStep:Math.max(1,Number(f.get("spawnExpansionStep"))||4),fillInnerGaps:f.get("spawnFillInnerGaps")==="on"}}); window.dispatchEvent(new CustomEvent("game-notify",{detail:{msg:"Configuração de nascimento de aldeias salva separadamente.",type:"success"}})); });
    ...
    if(view==="villages"){
      ...
      const vf=[["villageFilterSearch","rdg_v_search","change"],["villageFilterMin","rdg_v_min","change"],["villageFilterMax","rdg_v_max","change"],["villageFilterRole","rdg_v_role","change"],["villageFilterNoble","rdg_v_noble","change"],["villageFilterGroup","rdg_v_group","change"],["villageFilterOrder","rdg_v_order","change"]];vf.forEach(([id,key,ev])=>$("#"+id)?.addEventListener(ev,e=>{localStorage.setItem(key,e.target.value);render(true)})); $("#villageClearFilters")?.addEventListener("click",()=>{["rdg_v_search","rdg_v_min","rdg_v_max","rdg_v_role","rdg_v_noble","rdg_v_group","rdg_v_order"].forEach(k=>localStorage.removeItem(k));render(true)}); $("#villageSetGroup")?.addEventListener("click",()=>{const ids=[...document.querySelectorAll(".village-bulk-check:checked")].map(x=>x.value),name=$("#villageGroupName")?.value.trim();if(!ids.length||!name)return notifyUI("Selecione aldeias e informe um grupo.","warning");G.setVillageGroup(ids,name)}); $("#villageClearGroup")?.addEventListener("click",()=>G.setVillageGroup([...document.querySelectorAll(".village-bulk-check:checked")].map(x=>x.value),""));
    ...
    if(view==="reports"){ document.querySelectorAll(".report-summary").forEach(b=>b.onclick=e=>{e.preventDefault();const r=visibleReports().find(x=>String(x.id)===String(b.dataset.reportId));if(r)reportModal(r)}); $("#reportTypeFilter")?.addEventListener("change",e=>{localStorage.setItem("rdg_report_filter",e.target.value);render(true)}); $("#reportStateFilter")?.addEventListener("change",e=>{localStorage.setItem("rdg_report_state",e.target.value);render(true)}); document.querySelectorAll(".report-favorite").forEach(b=>b.onclick=()=>G.toggleReportFavorite(b.dataset.reportId)); $("#reportDeleteSelected")?.addEventListener("click",()=>G.deleteReports([...document.querySelectorAll(".report-check:checked")].map(x=>x.value))); document.querySelectorAll(".report-delete").forEach(b=>b.onclick=()=>G.deleteReport(b.dataset.reportId)); $("#reportPrefsToggle")?.addEventListener("click",()=>$("#reportPrefs")?.classList.toggle("d-none")); document.querySelectorAll(".report-pref").forEach(x=>x.onchange=()=>{const p=G.playerPreferences(),rt={...(p.reportTypes||{})};rt[x.value]=x.checked;G.updatePlayerPreferences({reportTypes:rt});}); }
    if(view==="commands"){ $("#commandFilter")?.addEventListener("change",e=>{localStorage.setItem("rdg_command_filter",e.target.value);render(true)}); $("#clearFinishedRecurring")?.addEventListener("click",()=>G.clearFinishedRecurring()); document.querySelectorAll(".recurring-delete").forEach(b=>b.onclick=()=>G.deleteRecurringAttack(b.dataset.id)); const rf=$("#recurringAttackForm");if(rf)rf.onsubmit=e=>{e.preventDefault();const f=new FormData(rf),targets=[...document.querySelectorAll(".recurring-target:checked")].map(o=>o.value);if(f.get("x")!==""&&f.get("y")!==""){const manual=G.villageAt(f.get("x"),f.get("y"));if(manual&&!targets.includes(manual.id))targets.push(manual.id);}if(!targets.length)return showInfo("Destino inválido","Selecione uma ou mais aldeias conhecidas ou informe coordenadas válidas.");const origins=[...document.querySelectorAll(".recurring-origin:checked")].map(x=>x.value),units={};Object.keys(C.units).forEach(k=>units[k]=Math.max(0,Number(f.get(`u-${k}`))||0));if(!Object.values(units).some(Boolean))return showInfo("Sem tropas","Informe ao menos uma unidade.");if(G.createRecurringAttack({origins,targets,units,intervalMs:(Number(f.get("interval"))||5)*60000,durationMs:(Number(f.get("duration"))||0)*60000})){showInfo("Programação criada","Os disparos verificarão as tropas disponíveis a cada execução.");render(true);}};{ const type=$("#recurringTargetType"),quad=$("#recurringTargetQuadrant"),search=$("#recurringTargetSearch"),limit=$("#recurringTargetLimit"),list=$("#recurringTargetList"),rows=[...document.querySelectorAll(".recurring-target-row")]; rows.sort((a,b)=>Number(a.dataset.distance)-Number(b.dataset.distance)).forEach(r=>list?.appendChild(r)); const apply=()=>{const t=type?.value||"all",q=quad?.value||"all",txt=(search?.value||"").trim().toLowerCase();rows.forEach(r=>r.style.display=(t==="all"||r.dataset.type===t)&&(q==="all"||r.dataset.q===q)&&(!txt||r.dataset.search.includes(txt))?"":"none")}; if(type)type.onchange=apply;if(quad)quad.onchange=apply;if(search)search.oninput=apply;const all=$("#recurringSelectVisible");if(all)all.onclick=()=>{const visible=rows.filter(r=>r.style.display!=="none").sort((a,b)=>Number(a.dataset.distance)-Number(b.dataset.distance)),n=Math.max(0,Number(limit?.value)||visible.length);rows.forEach(r=>r.querySelector(".recurring-target").checked=false);visible.slice(0,n).forEach(r=>r.querySelector(".recurring-target").checked=true);all.textContent=`Selecionados ${Math.min(n,visible.length)} mais próximos`;};apply(); } document.querySelectorAll(".recurring-action").forEach(b=>b.onclick=()=>{G.toggleRecurringAttack(b.dataset.id,b.dataset.action);render(true)}); const sf=$("#syncAttackForm"); if(sf) sf.onsubmit=e=>{e.preventDefault();const f=new FormData(sf),target=G.villageAt(f.get("x"),f.get("y"));if(!target)return notifyUI("Não existe aldeia no destino.","warning");const ratio=Number(f.get("ratio"))||.25,at=new Date(f.get("at")).getTime();let ok=0,fail=0;document.querySelectorAll(".sync-origin:checked").forEach(c=>{const v=G.state.villages[c.value],units=Object.fromEntries(Object.keys(C.units).map(k=>[k,Math.floor((v.units[k]||0)*ratio)]));G.scheduleAttackFrom(v.id,target.id,units,at,"arrival")?ok++:fail++;});if(ok){window.dispatchEvent(new Event("game-update"));notifyUI(`${ok} ataque(s) sincronizado(s).${fail?` ${fail} origem(ns) não puderam cumprir o horário/regras.`:""}`,fail?"warning":"success");}else notifyUI("Nenhuma origem pôde ser agendada.","warning");}; document.querySelectorAll(".support-withdraw").forEach(b=>b.onclick=()=>G.withdrawSupport(b.dataset.support)); document.querySelectorAll(".cancel-scheduled").forEach(b=>b.onclick=()=>G.cancelScheduled(b.dataset.movement)); }
    if(view==="hub"){
      $("#hubBackBtn")?.addEventListener("click",()=>{view=hubReturnView||"village";render(true)});
      $("#hubPauseBtn")?.addEventListener("click",()=>{G.togglePause();render(true)});
      $("#saveHubShortcuts")?.addEventListener("click",()=>{const quickMenu=[...document.querySelectorAll(".hub-quick-toggle:checked")].map(x=>x.value);G.updatePlayerPreferences({quickMenu});notifyUI("Atalhos da tela inicial atualizados.","success");render(true)});
      document.querySelectorAll("[data-hub-view]").forEach(b=>b.onclick=()=>{if(b.dataset.hubView==="admin"&&!requireAdmin())return;view=b.dataset.hubView;render(true)});
    }
    if (view === "map") bindMap();
  }
  const renderers = { hub, overview, village, army, villages, commands, market, simulator, ranking, achievements, map, reports, settings, admin };
  function render(force = false) {
    ...
    if (!mine.length && !isAdmin()) {
      ...
      const mode=(G.state.settings.respawnRules||{}).playerMode||"optional";
      if(mode==="automatic"){ G.restartCurrentPlayer(true); view="village"; render(true); return; }
      const restartBtn=$("#defeatRestartSame"); if(restartBtn) restartBtn.classList.toggle("d-none",mode==="disabled");
      const txt=$("#defeatModal .modal-body p"); if(txt) txt.textContent=mode==="disabled"?"Seu reino foi derrotado. O renascimento neste mundo está desativado pelo administrador.":"Seu reino foi derrotado neste mundo. Você pode recomeçar com uma nova aldeia inicial, sem herdar a evolução anterior.";
    ...
    const quickSet=new Set(G.playerPreferences?.().quickMenu||[]);
    document.querySelectorAll(".optional-quick").forEach(b=>b.classList.toggle("d-none",!quickSet.has(b.dataset.quickKey)));
    document
  ...
  window.addEventListener("game-notify", (e) => {
    ...
    $("#alerts").append(a);
    setTimeout(() => a.remove(), G.playerPreferences?.().compactNotifications ? 1800 : 3500);
  });
