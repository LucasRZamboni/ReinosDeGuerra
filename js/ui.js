(function () {
  ...
  function commands(){
    ...
    const rows=visible.sort((a,b)=>a.end-b.end).map(m=>{const from=G.state.villages[m.fromId],to=G.state.villages[m.targetId],mineFrom=mineIds.has(m.fromId),mineTo=mineIds.has(m.targetId);const kind=m.outbound?(mineFrom?"Ataque enviado":mineTo?"Ataque recebido":"Ataque mundial"):"Retorno";const reveal=admin||mineFrom||!m.outbound;const units=reveal?Object.entries(m.units||{}).filter(([,q])=>q).map(([k,q])=>`${C.units[k]?.icon||""}${q}`).join(" "):"Tropas ocultas";const owner=v=>v?.owner==="enemy"?`IA · ${v.aiProfile||"—"}`:v?.owner==="player"?"Jogador":"Livre";return `<tr><td><strong>${kind}</strong></td><td>${from?.name||"?"} (${from?.x}|${from?.y})<small>${admin?owner(from):""}</small></td><td>${to?.name||"?"} (${to?.x}|${to?.y})<small>${admin?owner(to):""}</small></td><td>${units}</td><td><span class="countdown" data-countdown="${m.end}">${remaining(m.end)}</span></td></tr>`}).join("");
