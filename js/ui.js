(function () {
  ...
  function villages() {
    const mine = G.owned();
    const totalPoints = mine.reduce((n,v)=>n+G.points(v),0);
    const rows = mine.map(v=>{
      const cap=G.cap(v), pop=G.population(v), popCap=G.popCap(v);
      const bq=v.buildQueue?.[0], tq=v.trainQueue?.[0];
      const possible=Object.entries(C.buildings).filter(([k,b])=>G.meets(v,b.requires)&&(v.buildings[k]||0)<b.maxLevel).slice(0,6);
      const troops=Object.entries(v.units||{}).filter(([,q])=>q>0).map(([k,q])=>`${C.units[k]?.icon||""}${fmt(q)}`).join(" ")||"—";
      return `<tr data-village-row="${v.id}"><td><strong>${v.name}</strong><div class="small text-secondary">${v.x}|${v.y} · ${G.points(v)} pts</div></td><td>🪵 ${fmt(v.resources.wood)}/${fmt(cap)}<br>🧱 ${fmt(v.resources.clay)}/${fmt(cap)}<br>⛓ ${fmt(v.resources.iron)}/${fmt(cap)}</td><td>👥 ${pop}/${popCap}<div class="small village-troop-summary">${troops}</div></td><td>${bq?`${C.buildings[bq.building].name} <span data-countdown="${bq.end}">${remaining(bq.end)}</span>`:"<span class='text-secondary'>Fila vazia</span>"}<div class="mt-1"><select class="form-select form-select-sm village-quick-build" data-id="${v.id}"><option value="">Construção rápida…</option>${possible.map(([k,b])=>`<option value="${k}">${b.name} → ${(v.buildings[k]||0)+1}</option>`).join("")}</select></div></td><td>${tq?`${C.units[tq.unit].name} ${tq.trained||0}/${tq.amount}`:"<span class='text-secondary'>Fila vazia</span>"}<div class="input-group input-group-sm mt-1"><select class="form-select village-quick-unit" data-id="${v.id}"><option value="">Treinar…</option>${Object.entries(C.units).filter(([k,u])=>G.meets(v,u.requires)).map(([k,u])=>`<option value="${k}">${u.name}</option>`).join("")}</select><input class="form-control village-quick-amount" data-id="${v.id}" type="number" min="1" value="10" style="max-width:80px"><button class="btn btn-outline-warning village-quick-train" data-id="${v.id}" type="button">OK</button></div></td><td><div class="d-flex flex-wrap gap-1"><button class="btn btn-sm btn-warning village-open" data-id="${v.id}">Abrir</button><button class="btn btn-sm btn-outline-light village-map" data-id="${v.id}">Mapa</button></div></td></tr>`;
    }).join("");
    $("#view-villages").innerHTML=`<div class="section-title"><div><div class="eyebrow">Gestão do reino</div><h2>Minhas aldeias</h2></div><div class="d-flex gap-2"><span class="badge text-bg-warning">★ ${fmt(totalPoints)} pontos</span><span class="badge text-bg-secondary">♜ ${mine.length} aldeias</span></div></div><section class="panel p-3"><div class="row g-2 mb-3"><div class="col-md-6"><input id="villageSearch" class="form-control" placeholder="Buscar por nome ou coordenada"></div><div class="col-md-3"><select id="villageFilter" class="form-select"><option value="all">Todas</option><option value="build-empty">Fila de construção vazia</option><option value="train-empty">Fila de treino vazia</option><option value="storage">Recursos ≥ 90%</option></select></div></div><div class="table-responsive villages-overview"><table class="table table-dark table-hover align-middle"><thead><tr><th>Aldeia</th><th>Recursos</th><th>Tropas / Pop.</th><th>Construção</th><th>Treino</th><th>Ações</th></tr></thead><tbody>${rows}</tbody></table></div></section>`;
  }
  function admin() {
  ...
  const renderers = { village, army, villages, map, reports, settings, admin };
