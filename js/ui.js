(function () {
  ...
  function overview(){
    const mine=G.owned(), pts=mine.reduce((n,v)=>n+G.points(v),0), prod={wood:0,clay:0,iron:0}, troops={}; let pop=0,cap=0,builds=0,trains=0,incoming=0;
    mine.forEach(v=>{["wood","clay","iron"].forEach(r=>prod[r]+=G.prod(v,r)*60);Object.entries(v.units||{}).forEach(([k,q])=>troops[k]=(troops[k]||0)+q);pop+=G.population(v);cap+=G.popCap(v);builds+=(v.buildQueue||[]).length;trains+=(v.trainQueue||[]).length;}); const ids=new Set(mine.map(v=>v.id)); incoming=G.state.movements.filter(m=>m.outbound&&m.kind!=="support"&&ids.has(m.targetId)).length;
