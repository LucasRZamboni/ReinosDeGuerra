(function () {
  ...
  function ranking(){
    ...
    const stats={};Object.values(G.state.villages).filter(v=>v.owner==="player"&&v.ownerId).forEach(v=>{const x=stats[v.ownerId]||(stats[v.ownerId]={id:v.ownerId,points:0,villages:0});x.points+=G.points(v);x.villages++});
    const list=Object.values(stats).sort((a,b)=>b.points-a.points); const rows=list.map((x,i)=>`<tr class="ranking-player" data-player="${x.id}"><td>#${i+1}</td><td><strong>${names[x.id]||x.id}</strong></td><td>${fmt(x.points)}</td><td>${x.villages}</td><td>${Math.max(0,x.villages-1)}</td></tr>`).join("");
