(function () {
  ...
  function villages() {
    ...
    $("#view-villages").innerHTML=`<div class="section-title"><div><div class="eyebrow">Gestão do reino</div><h2>Minhas aldeias</h2></div><div class="d-flex gap-2 flex-wrap"><span class="badge text-bg-warning">★ ${fmt(totalPoints)} pontos</span><span class="badge text-bg-secondary">♜ ${mine.length} aldeias</span></div></div><section class="panel p-3 mb-3 village-bulk-toolbar"><div class="d-flex flex-wrap gap-2 align-items-end"><button id="villageSelectAll" class="btn btn-sm btn-outline-light">Selecionar todas</button><button id="villageClearAll" class="btn btn-sm btn-outline-secondary">Limpar</button><label class="flex-grow-1">Construir<select id="villageBulkBuilding" class="form-select form-select-sm"><option value="">Escolha…</option>${buildOpts}</select></label><button id="villageBulkBuild" class="btn btn-sm btn-warning">Construir selecionadas</button><button class="btn btn-sm btn-outline-warning village-bulk-preset" data-preset="defense">Treinar Defesa</button><button class="btn btn-sm btn-outline-danger village-bulk-preset" data-preset="attack">Treinar Ataque</button><button class="btn btn-sm btn-outline-info village-bulk-preset" data-preset="custom">Personalizado</button></div><div class="small text-secondary mt-2">As ações respeitam recursos, população, requisitos, níveis máximos e regras de fila de cada aldeia.</div></section><div class="village-management-grid">${cards||'<p class="text-secondary">Nenhuma aldeia.</p>'}</div>`;
  }
  function commands(){
    const pid=G.currentPlayerId(), mineIds=new Set(G.owned().map(v=>v.id));
    const rows=G.state.movements.filter(m=>mineIds.has(m.fromId)||mineIds.has(m.targetId)).sort((a,b)=>a.end-b.end).map(m=>{const from=G.state.villages[m.fromId],to=G.state.villages[m.targetId],mineFrom=mineIds.has(m.fromId),mineTo=mineIds.has(m.targetId);const kind=m.outbound?(mineFrom?"Ataque enviado":"Ataque recebido"):"Retorno";const reveal=mineFrom||!m.outbound;const units=reveal?Object.entries(m.units||{}).filter(([,q])=>q).map(([k,q])=>`${C.units[k]?.icon||""}${q}`).join(" "):"Tropas ocultas";return `<tr><td><strong>${kind}</strong></td><td>${from?.name||"?"} (${from?.x}|${from?.y})</td><td>${to?.name||"?"} (${to?.x}|${to?.y})</td><td>${units}</td><td><span class="countdown" data-countdown="${m.end}">${remaining(m.end)}</span></td></tr>`}).join("");
    $("#view-commands").innerHTML=`<div class="section-title"><div><div class="eyebrow">Movimentações militares</div><h2>Comandos</h2></div></div><section class="panel p-3"><div class="table-responsive"><table class="table table-dark align-middle commands-table"><thead><tr><th>Tipo</th><th>Origem</th><th>Destino</th><th>Tropas</th><th>Chegada</th></tr></thead><tbody>${rows||'<tr><td colspan="5" class="text-secondary">Nenhum comando ativo.</td></tr>'}</tbody></table></div></section>`;
  }
  function ranking(){
    ...
    const stats={};Object.values(G.state.villages).forEach(v=>{
      if(v.owner==="player"&&v.ownerId){const x=stats[v.ownerId]||(stats[v.ownerId]={id:v.ownerId,points:0,villages:0,type:"player"});x.points+=G.points(v);x.villages++;}
      if(v.owner==="enemy"){const id=v.aiId||`ai-${v.id}`, x=stats[id]||(stats[id]={id,points:0,villages:0,type:"enemy",name:v.name||"Inimigo IA"});x.points+=G.points(v);x.villages++;}
    });
    const list=Object.values(stats).sort((a,b)=>b.points-a.points); const rows=list.map((x,i)=>`<tr class="ranking-player" data-player="${x.id}"><td>#${i+1}</td><td><strong>${x.type==="enemy"?`🤖 ${x.name}`:(names[x.id]||x.id)}</strong>${x.type==="enemy"?'<div class="small text-secondary">Jogador IA</div>':''}</td><td>${fmt(x.points)}</td><td>${x.villages}</td><td>${Math.max(0,x.villages-1)}</td></tr>`).join("");
    $("#view-ranking").innerHTML=`<div class="section-title"><div><div class="eyebrow">Classificação do mundo</div><h2>Ranking</h2></div></div><section class="panel p-3"><div class="table-responsive"><table class="table table-dark table-hover"><thead><tr><th>#</th><th>Jogador</th><th>Pontos</th><th>Aldeias</th><th>Conquistas</th></tr></thead><tbody>${rows}</tbody></table></div><div class="small text-secondary">Perfis públicos mostram somente pontuação, aldeias e conquistas; recursos e tropas permanecem privados.</div></section>`;
  }
  function admin() {
  ...
  function bindDynamic() {
    ...
    if (view === "settings") {
      ...
      const saveSettingsTab = (tab) => {
        ...
        if (tab === "speed") {
          ...
          G.updateSettings({ difficulty:$("#newDifficulty").value, objective:$("#newObjective").value, freeVillagesTrainTroops:f.get("freeVillagesTrainTroops")==="on" });
        }
    ...
    if (view === "admin") {
      ...
      if($("#adminBulkForm")) $("#adminBulkForm").onsubmit=e=>{e.preventDefault(); const ids=bulkChecks().filter(x=>x.checked).map(x=>x.value); if(!ids.length)return alert("Selecione ao menos uma aldeia."); const f=new FormData(e.currentTarget), data={}; const res=f.get("bulkResources"); if(res)data.resources={wood:res,clay:res,iron:res}; const bonus=f.get("bulkBonus"); if(bonus)data.bonusType=bonus; const bp=f.get("bulkBuildings"); if(bp==="initial")data.buildings={...G.state.settings.initialBuildingLevels}; if(bp==="half")data.buildings=Object.fromEntries(Object.entries(C.buildings).map(([k,b])=>[k,Math.floor(b.maxLevel*(G.state.settings.buildingPresets?.halfRatio||.5))])); if(bp==="custom")data.buildings={...(G.state.settings.buildingPresets?.custom||G.state.settings.initialBuildingLevels)}; if(bp==="max")data.buildings=Object.fromEntries(Object.entries(C.buildings).map(([k,b])=>[k,b.maxLevel])); const tp=f.get("bulkTroops"); if(tp==="defense")data.units={...C.adminPresets.defenseTroops}; if(tp==="attack")data.units={...C.adminPresets.attackTroops}; if(tp==="zero")data.units=Object.fromEntries(Object.keys(C.units).map(k=>[k,0])); if(!Object.keys(data).length)return alert("Escolha ao menos uma alteração."); if(confirm(`Aplicar as alterações a ${ids.length} aldeia(s)?`)){G.adminBulkUpdate(ids,data); const finish=f.get("bulkFinish"),owner=f.get("bulkOwner"); if(finish)G.adminBulkFinish(ids,finish); if(owner)G.adminBulkOwner(ids,owner); render(true);}};
      ...
      $("#adminWorldForm").onsubmit = (e) => {
        ...
        G.updateSettings({
          ...
          enemyRules:{...(G.state.settings.enemyRules||{}),canAttackPlayers:f.get("enemyAttackPlayers")==="on",canAttackBarbarians:f.get("enemyAttackBarbs")==="on",canConquer:f.get("enemyConquer")==="on",maxVillagesPerEnemy:Number(f.get("enemyMaxVillages"))||12,attackRadius:Number(f.get("enemyRadius"))||25},
        });
      };
      $("#saveSpawnSettings")?.addEventListener("click",()=>{ const f=new FormData($("#adminWorldForm")); G.updateSettings({barbarianSpawn:{enabled:f.get("spawnEnabled")==="on",intervalMinutes:Math.max(1,Number(f.get("spawnInterval"))||30),maxNewVillages:Math.max(0,Number(f.get("spawnMax"))||0),perCycle:Math.max(1,Number(f.get("spawnPerCycle"))||1),bonusChance:Math.max(0,Math.min(100,Number(f.get("spawnBonus"))||0))}}); window.dispatchEvent(new CustomEvent("game-notify",{detail:{msg:"Configuração de nascimento de aldeias salva separadamente.",type:"success"}})); });
      ...
      $("#adminCustomTroops").onclick = () => applyArmyPreset(C.adminPresets.customTroops);
  ...
  const renderers = { village, army, villages, commands, ranking, map, reports, settings, admin };
